...
import com.zifuji.cloud.server.business.module.webUser.mapper.UserMapper;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<WebMenuServiceVo> getMenu() {
        List<WebMenuServiceVo> webMenuServiceVoList = new ArrayList<>();
                ...
                if (ObjectUtil.isNotEmpty(webRoleMenuEntityList)) {
                    ...
                    if (ObjectUtil.isNotEmpty(webMenuEntityList)) {
                        webMenuServiceVoList = webMenuEntityList.stream().map(webMenuEntity -> {
                            WebMenuServiceVo bo = new WebMenuServiceVo();
    ...
    @Override
    public DrawCaptchaServiceVo drawCaptcha() {
        DrawCaptchaServiceVo drawCaptchaServiceVo = new DrawCaptchaServiceVo();
        String redisUuid = StrUtil.uuid();
        drawCaptchaServiceVo.setRedisUuid(redisUuid);
        LineCaptcha lineCaptcha = CaptchaUtil.createLineCaptcha(200, 100);
        RandomGenerator randomGenerator = new RandomGenerator(BaseConstant.STRING_C, 4);
        lineCaptcha.setGenerator(randomGenerator);
        lineCaptcha.createCode();
        drawCaptchaServiceVo.setLineCaptcha(lineCaptcha);
        drawCaptchaServiceVo.setImgBytes(lineCaptcha.getImageBytes());
        stringRedisTemplate.opsForValue().set(redisUuid, lineCaptcha.getCode(), 60 * 30, TimeUnit.SECONDS);
        return drawCaptchaServiceVo;
    }

    @Override
    public String login(LoginServiceMo loginServiceMo) {
        // 先校验验证码
        if (RedisUtil.checkCodeAndValue(stringRedisTemplate, loginServiceMo.getRedisUuid(), loginServiceMo.getRedisValue())) {
            // 通过用户名查询数据库记录
            QueryWrapper<WebUserEntity> webUserEntityQueryWrapper = new QueryWrapper<>();
            webUserEntityQueryWrapper.lambda().eq(WebUserEntity::getUserName, loginServiceMo.getUserName());
            ...
            if (ObjectUtil.isNotNull(webUserEntity)) {
                ...
                if (bCryptPasswordEncoder.matches(loginServiceMo.getPassWord(), webUserEntity.getPassWord())) {
    ...
    @Override
    public String register(RegisterServiceMo registerServiceMo) {
        if (RedisUtil.checkCodeAndValue(stringRedisTemplate, registerServiceMo.getRedisUuid(), registerServiceMo.getRedisValue())) {
            return null;
        } else {
    ...
    @Override
    public IPage<WebUserServiceVo> queryPageWebUser() {
        ...
        return page.convert(webUserEntity -> {
            WebUserServiceVo webUserServiceVo = new WebUserServiceVo();
            BeanUtil.copyProperties(webUserEntity, webUserServiceVo);
            return webUserServiceVo;
        });
    }

    @Override
    public IPage<WebMenuServiceVo> queryPageWebMenu() {
        return null;
    }
}
