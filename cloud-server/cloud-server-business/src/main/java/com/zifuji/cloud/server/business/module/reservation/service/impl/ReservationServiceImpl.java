...
public class ReservationServiceImpl implements ReservationService {
	...
	@Override
	public ReservationApplyMainControllerVo saveReservationSettingMain(
			ReservationSettingMainControllerMo reservationSettingMainMo) {
		// 判断星期工作日设置的正确性
		...
		if (ObjectUtil.isNull(reservationSettingMainMo.getTableId())) {
			...
			for (ReservationSettingWorkDayMo reservationSettingWorkDayMo : reservationSettingMainMo
					.getReservationSettingWorkDayMoList()) {
				ReservationSettingWorkDayEntity reservationSettingWorkDayEntity = new ReservationSettingWorkDayEntity();
				reservationSettingWorkDayEntity.setSettingMainId(reservationSettingMainEntity.getTableId());
			...
			for (ReservationSettingWorkTimeMo reservationSettingWorkTimeMo : reservationSettingMainMo
					.getReservationSettingWorkTimeMoList()) {
				ReservationSettingWorkTimeEntity reservationSettingWorkTimeEntity = new ReservationSettingWorkTimeEntity();
				reservationSettingWorkTimeEntity.setSettingMainId(reservationSettingMainEntity.getTableId());
...
//                reservationSettingSpeDayEntity.setSettingMainId(reservationSettingMainEntity.getTableId());
...
//                reservationSettingSpeTimeEntity.setSettingMainId(reservationSettingMainEntity.getTableId());
...
//            }
	...
	void createDetailSetting(Long id, List<ReservationSettingWorkDayMo> reservationSettingWorkDayMoList,
			List<ReservationSettingWorkTimeMo> reservationSettingWorkTimeMoList) {
		...
		for (int i = 0; i < 365; i++) {
			...
			reservationSettingDayRecordEntity
					.setDayStatus(reservationSettingWorkDayMoList.get(week.getValue() - 1).getWorkFlag());
			...
			for (ReservationSettingWorkTimeMo reservationSettingWorkTimeMo : reservationSettingWorkTimeMoList) {
				...
				reservationSettingTimeRecordEntity.setDayRecordId(reservationSettingDayRecordEntity.getTableId());
				reservationSettingTimeRecordEntity.setTimeRange(reservationSettingWorkTimeMo.getStartRange() + "-"
						+ reservationSettingWorkTimeMo.getEndRange());
	...
	public Boolean openReservation(ReservationSettingMainControllerMo reservationSettingMainMo) {
		if (ObjectUtil.isNull(reservationSettingMainMo) || "0".equals(reservationSettingMainMo.getTableId())) {
			throw new Exception20000("");
		}
		ReservationSettingMainEntity reservationSettingMainEntity = reservationSettingMainEntityService
				.getById(reservationSettingMainMo.getTableId());
	...
	private int getDayTimeHaveNum(String code, String day, String startRange, String endRange) {
		// time剩余预约数
		String str = stringRedisTemplate.opsForValue()
				.get("reservation" + code + day + startRange + endRange + "have" + "num");
		if (StrUtil.isBlank(str)) {
			ReservationDayBo bo = getRedisDay(code, day);
			for (ReservationTimeBo reservationTimeBo : bo.getReservationTimeBoList()) {
				if (StrUtil.equals(startRange, reservationTimeBo.getStartRange())
						&& StrUtil.equals(endRange, reservationTimeBo.getEndRange())) {
					stringRedisTemplate.opsForValue().set(
							"reservation" + code + day + startRange + endRange + "have" + "num",
							"" + reservationTimeBo.getHaveNum());
	...
	private ReservationDayBo getDbDay(String code, String day) {
		...
		ReservationSettingMainEntity reservationSettingMainEntity = reservationSettingMainEntityService
				.getOne(reservationSettingMainEntityQueryWrapper);
		// 查看day是不是工作日
		String workFlag = "";
		...
		reservationSettingSpeWorkEntityQueryWrapper.lambda()
				.eq(ReservationSettingSpeDayEntity::getSettingMainId, reservationSettingMainEntity.getTableId())
				.eq(ReservationSettingSpeDayEntity::getSpeDay, day);
		ReservationSettingSpeDayEntity reservationSettingSpeDayEntity = reservationSettingSpeDayEntityService
				.getOne(reservationSettingSpeWorkEntityQueryWrapper);
		if (ObjectUtil.isNull(reservationSettingSpeDayEntity)) {
			...
			reservationSettingWorkDayEntityQueryWrapper.lambda()
					.eq(ReservationSettingWorkDayEntity::getSettingMainId, reservationSettingMainEntity.getTableId())
					.eq(ReservationSettingWorkDayEntity::getDayNo, week.getValue());
			ReservationSettingWorkDayEntity reservationSettingWorkDayEntity = reservationSettingWorkDayEntityService
					.getOne(reservationSettingWorkDayEntityQueryWrapper);
		...
		if (StrUtil.equals(workFlag, "工作日")) {
			...
			reservationSettingSpeTimeEntityQueryWrapper.lambda()
					.eq(ReservationSettingSpeTimeEntity::getSettingMainId, reservationSettingMainEntity.getTableId())
					.eq(ReservationSettingSpeTimeEntity::getSpeDay, day)
					.orderByAsc(ReservationSettingSpeTimeEntity::getSpeStartRange);
			List<ReservationSettingSpeTimeEntity> reservationSettingSpeTimeEntityList = reservationSettingSpeTimeEntityService
					.list(reservationSettingSpeTimeEntityQueryWrapper);
			if (ObjectUtil.isEmpty(reservationSettingSpeTimeEntityList)) {
				...
				reservationSettingWorkTimeEntityQueryWrapper.lambda()
						.eq(ReservationSettingWorkTimeEntity::getSettingMainId,
								reservationSettingMainEntity.getTableId())
						.orderByAsc(ReservationSettingWorkTimeEntity::getStartRange);
				List<ReservationSettingWorkTimeEntity> reservationSettingWorkTimeEntityList = reservationSettingWorkTimeEntityService
						.list(reservationSettingWorkTimeEntityQueryWrapper);
				reservationTimeBoList = reservationSettingWorkTimeEntityList.stream()
						.map(reservationSettingWorkTimeEntity -> {
				...
				reservationTimeBoList = reservationSettingSpeTimeEntityList.stream()
						.map(reservationSettingSpeTimeEntity -> {
							ReservationTimeBo reservationTimeBo = new ReservationTimeBo();
							reservationTimeBo.setDayName(day);
			...
			for (ReservationTimeBo reservationTimeBo : reservationTimeBoList) {
				...
				reservationApplyRecordEntityQueryWrapper.lambda()
						.eq(ReservationApplyRecordEntity::getSettingMainId, reservationSettingMainEntity.getTableId())
